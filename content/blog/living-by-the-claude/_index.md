...
But that doesn't count, so I flexed my Science Muscles and began downloading simulation code from a rival research group, in the hope that I could extend on their work for some fast and impressive outputs. After chanting the right git incantations (provided from Claude, of course), I had the repo going and used Claude to explore its capabilities - really an awesome tool to have alongside the paper. I should note that the repo was quite a bit better engineered than I would expect of the typical paper simulation. 